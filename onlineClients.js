...
wss.on('connection', ws => {
    ...
    // Send structured userCount to all clients (previously sent raw number)
    wss.clients.forEach(client=>{
        try {
            client.send(JSON.stringify({ type: 'userCount', count: totalUser }));
        } catch(e){ /* ignore */ }
    })
    // Send the current user count to the newly connected client 
    try { ws.send(JSON.stringify({ type: 'userCount', count: totalUser })); } catch(e) {}
    ...
    ws.on('message',async (message) => {
      ...
      if(msg.type == 'message'){
          ...
          const parsedData = msg; // already parsed above
          console.log(parsedData); // Log the entire parsed object
          if (parsedData && parsedData.gameId !== undefined) {
            const gameId = parsedData.gameId;
            console.log('Game ID:', gameId);
        ...
        console.log(msg)
        // keep your games array usage but broadcast a single object not array
        ...
        // Broadcast a single JSON object for move instead of an array
        wss.clients.forEach(client=>{
          try {
            // Send single object with clear fields
            client.send(JSON.stringify({ type: 'move', msg: msg.message, gameId: msg.gameId, from: msg.from, to: msg.to }));
          } catch(e){ /* ignore */ }
        })
      }
      else{
    ...
    ws.on('close', async() => {
        ...
        // On WebSocket disconnect, save the games array to the database
        wss.clients.forEach(client=>{
            try { client.send(JSON.stringify({ type: 'userCount', count: totalUser })); } catch(e) {}
        })
    })
});